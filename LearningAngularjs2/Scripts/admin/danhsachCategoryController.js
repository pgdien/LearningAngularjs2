myApp.controller("danhsachCategoryController", ['$scope', '$http', '$window','Category', 'MenuMultiLevel', 'uiGridConstants', function ($scope, $http, $window, MenuMultiLevel, uiGridConstants) {
    $scope.category = {};
    $scope.categories = [];
    $scope.gridOptions = {};
    ...
    .success(function (data) {
        $scope.gridOptions.data = Category.getTable(data);
    });

    //Chỉnh cột
    $scope.gridOptions.columDefs =
        [
            {
                displayName: "STT",
                name: 'stt',
    ...
    $scope.gridOptions.multiLevel = false;
